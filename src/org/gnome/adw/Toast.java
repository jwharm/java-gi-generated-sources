...
public class Toast extends org.gtk.gobject.Object {
    ...
    public SignalHandle onDismissed(DismissedHandler handler) {
        try {
            ...
            MethodHandle methodHandle = MethodHandles.lookup().findStatic(JVMCallbacks.class, "signalToastDismissed", methodType);
            FunctionDescriptor descriptor = FunctionDescriptor.ofVoid(ValueLayout.ADDRESS, ValueLayout.ADDRESS);
            MemorySegment nativeSymbol = Linker.nativeLinker().upcallStub(methodHandle, descriptor, Interop.getScope());
            long handlerId = gtk_h.g_signal_connect_data(handle(), Interop.allocateNativeString("dismissed").handle(), nativeSymbol, intSegment, MemoryAddress.NULL, 0);

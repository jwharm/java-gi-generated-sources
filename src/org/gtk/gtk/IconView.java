 ...
 * (depending on the selection mode, see {@link org.gtk.gtk.IconView<code>#setSelectionMode</code> ).
 ...
 * ignore the tree&<code>#8217</code> s branches.
 * <p>
 * <h1>CSS nodes</h1>
 * <p><pre>
 * iconview.view
...
public class IconView extends Widget implements Accessible, Buildable, CellLayout, ConstraintTarget, Scrollable {
     ...
     * method sets <code>GtkIconView</code>:reorderable to <code>false</code>
     */
     ...
     * If the cursor isn&<code>#8217</code> t currently set, then *@path will be <code>null</code> 
     * If no cell currently has focus, then *@cell will be <code>null</code> 
     * <p>
     * The returned <code>GtkTreePath</code> must be freed with gtk_tree_path_free().
     */
    ...
    /**
     * Returns the column of @icon_view&<code>#8217</code> s model which is being used for
     * displaying tooltips on @icon_view&<code>#8217</code> s rows.
     */
     ...
     * to {@link org.gtk.gtk.SelectionMode<code>#MULTIPLE</code>
     */
        ...
        try {
            gtk_h.gtk_icon_view_selected_foreach(handle(), 
                    CLinker.systemCLinker().upcallStub(
                        MethodHandles.lookup().findStatic(JVMCallbacks.class, "cbIconViewForeachFunc",
     ...
     * useful when you want to focus the user&<code>#8217</code> s attention on a particular item.
     ...
     * around each of the icon view&<code>#8217</code> s items.
     */
     ...
     * developer can listen to these changes by connecting to the model&<code>#39</code> s
     ...
     * for you. @column should be set to the column in @icon_view&<code>#8217</code> s model
    ...
    public SignalHandle onActivateCursorItem(ActivateCursorItemHandler handler) {
        try {
            ...
            long handlerId = gtk_h.g_signal_connect_data(handle(), Interop.allocateNativeString("activate-cursor-item").handle(), nativeSymbol, intSegment, MemoryAddress.NULL, 0);
     ...
     * clicks an item with the &<code>#34</code> activate-on-single-click&<code>#34</code>  property set
     * to <code>false</code>  or when the user single clicks an item when the
     * &<code>#34</code> activate-on-single-click&<code>#34</code>  property set to <code>true</code>  It is also
    ...
    public SignalHandle onItemActivated(ItemActivatedHandler handler) {
        try {
            ...
            NativeSymbol nativeSymbol = CLinker.systemCLinker().upcallStub(methodHandle, descriptor, Interop.getScope());
            long handlerId = gtk_h.g_signal_connect_data(handle(), Interop.allocateNativeString("item-activated").handle(), nativeSymbol, intSegment, MemoryAddress.NULL, 0);
            return new SignalHandle(handle(), handlerId);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
     ...
     * - PageUp/PageDown which move by &<code>#34</code> pages&<code>#34</code> 
    ...
    public SignalHandle onSelectCursorItem(SelectCursorItemHandler handler) {
        try {
            ...
            FunctionDescriptor descriptor = FunctionDescriptor.ofVoid(ValueLayout.ADDRESS, ValueLayout.ADDRESS);
            NativeSymbol nativeSymbol = CLinker.systemCLinker().upcallStub(methodHandle, descriptor, Interop.getScope());
            long handlerId = gtk_h.g_signal_connect_data(handle(), Interop.allocateNativeString("select-cursor-item").handle(), nativeSymbol, intSegment, MemoryAddress.NULL, 0);
            return new SignalHandle(handle(), handlerId);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    ...
    public SignalHandle onToggleCursorItem(ToggleCursorItemHandler handler) {
        try {
            ...
            FunctionDescriptor descriptor = FunctionDescriptor.ofVoid(ValueLayout.ADDRESS, ValueLayout.ADDRESS);
            NativeSymbol nativeSymbol = CLinker.systemCLinker().upcallStub(methodHandle, descriptor, Interop.getScope());
            long handlerId = gtk_h.g_signal_connect_data(handle(), Interop.allocateNativeString("toggle-cursor-item").handle(), nativeSymbol, intSegment, MemoryAddress.NULL, 0);
            return new SignalHandle(handle(), handlerId);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }

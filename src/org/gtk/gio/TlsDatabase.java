...
/**
 * {@link org.gtk.gio.TlsDatabase} is used to look up certificates and other information
 * from a certificate or key store. It is an abstract base class which
 * TLS library specific subtypes override.
 * 
 * A {@link org.gtk.gio.TlsDatabase} may be accessed from multiple threads by the TLS backend.
 * All implementations are required to be fully thread-safe.
 * 
 * Most common client applications will not directly interact with
 * {@link org.gtk.gio.TlsDatabase}  It is used internally by {@link org.gtk.gio.TlsConnection}
 */
public class TlsDatabase extends org.gtk.gobject.Object {
     ...
     * cases where the database cannot create a handle for a certificate, <code>null</code> will be returned.
     * 
     ...
     * g_tls_database_create_certificate_handle() on a {@link org.gtk.gio.TlsDatabase} object of
    ...
    public void lookupCertificateForHandleAsync(java.lang.String handle, TlsInteraction interaction, TlsDatabaseLookupFlags flags, Cancellable cancellable, AsyncReadyCallback callback) {
        try {
            gtk_h.g_tls_database_lookup_certificate_for_handle_async(handle(), Interop.allocateNativeString(handle).handle(), interaction.handle(), flags.getValue(), cancellable.handle(), 
     ...
     * {@link org.gtk.gio.TlsCertificate} issuer property of @certificate is not modified, and
     ...
     * the certification path (though GLib&<code>#39</code> s TLS backends may not follow the
    ...
    public void lookupCertificateIssuerAsync(TlsCertificate certificate, TlsInteraction interaction, TlsDatabaseLookupFlags flags, Cancellable cancellable, AsyncReadyCallback callback) {
        try {
            gtk_h.g_tls_database_lookup_certificate_issuer_async(handle(), certificate.handle(), interaction.handle(), flags.getValue(), cancellable.handle(), 
                    CLinker.systemCLinker().upcallStub(
                        MethodHandles.lookup().findStatic(JVMCallbacks.class, "cbAsyncReadyCallback",
                            MethodType.methodType(void.class, MemoryAddress.class, MemoryAddress.class, MemoryAddress.class)),
                        FunctionDescriptor.ofVoid(ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS),
    ...
    public void lookupCertificatesIssuedByAsync(byte[] issuerRawDn, TlsInteraction interaction, TlsDatabaseLookupFlags flags, Cancellable cancellable, AsyncReadyCallback callback) {
        try {
            gtk_h.g_tls_database_lookup_certificates_issued_by_async(handle(), new MemorySegmentReference(Interop.getAllocator().allocateArray(ValueLayout.JAVA_BYTE, issuerRawDn)).handle(), interaction.handle(), flags.getValue(), cancellable.handle(), 
                    CLinker.systemCLinker().upcallStub(
                        MethodHandles.lookup().findStatic(JVMCallbacks.class, "cbAsyncReadyCallback",
                            MethodType.methodType(void.class, MemoryAddress.class, MemoryAddress.class, MemoryAddress.class)),
                        FunctionDescriptor.ofVoid(ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.ADDRESS),
     ...
     * of a TLS session.
     * 
     * @chain is a chain of {@link org.gtk.gio.TlsCertificate} objects each pointing to the next
     * certificate in the chain by its {@link org.gtk.gio.TlsCertificate} issuer property.
     * 
     * @purpose describes the purpose (or usage) for which the certificate
     * is being used. Typically @purpose will be set to <code>G_TLS_DATABASE_PURPOSE_AUTHENTICATE_SERVER</code> which means that the certificate is being used to authenticate a server
     * (and we are acting as the client).
     * 
     * The @identity is used to ensure the server certificate is valid for
     * the expected peer identity. If the identity does not match the
     * certificate, {@link org.gtk.gio.TlsCertificateFlags<code>#BAD_IDENTITY</code>  will be set in the
     * return value. If @identity is <code>null</code>  that bit will never be set in
     * the return value. The peer identity may also be used to check for
     * pinned certificates (trust exceptions) in the database. These may
     * override the normal verification process on a host-by-host basis.
     * 
     * Currently there are no @flags, and {@link org.gtk.gio.TlsDatabaseVerifyFlags<code>#NONE</code>  should be
     * used.
     * 
     ...
     * {@link org.gtk.gio.TlsCertificateFlags<code>#GENERIC_ERROR</code>  and @error will be set accordingly.
     ...
     * 
     * Prior to GLib 2.48, GLib&<code>#39</code> s default TLS backend modified @chain to
     * represent the certification path built by {@link org.gtk.gio.TlsDatabase} during
     * certificate verification by adjusting the {@link org.gtk.gio.TlsCertificate} issuer
     * property of each certificate in @chain. Since GLib 2.48, this no
     * longer occurs, so you cannot rely on {@link org.gtk.gio.TlsCertificate} issuer to
     * represent the actual certification path used during certificate
     * verification.
     * 
     * Because TLS session context is not used, {@link org.gtk.gio.TlsDatabase} may not
     ...
     * {@link org.gtk.gio.TlsCertificateFlags<code>#GENERIC_ERROR</code>  and @error will be set

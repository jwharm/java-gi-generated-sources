...
public class FileInputStream extends InputStream implements Seekable {
    ...
    public FileInputStream(io.github.jwharm.javagi.interop.Proxy proxy) {
        super(proxy);
    }
    
    /** Cast object to FileInputStream */
    public static FileInputStream castFrom(org.gtk.gobject.Object gobject) {
        return new FileInputStream(gobject.getProxy());
    }
    ...
    public FileInfo queryInfo(java.lang.String attributes, Cancellable cancellable) throws io.github.jwharm.javagi.interop.GErrorException {
        ...
        var RESULT = io.github.jwharm.javagi.interop.jextract.gtk_h.g_file_input_stream_query_info(HANDLE(), Interop.getAllocator().allocateUtf8String(attributes), cancellable.HANDLE(), GERROR);
    ...
    public FileInfo queryInfoFinish(AsyncResult result) throws io.github.jwharm.javagi.interop.GErrorException {
        MemorySegment GERROR = io.github.jwharm.javagi.interop.jextract.GError.allocate(Interop.getScope());
        var RESULT = io.github.jwharm.javagi.interop.jextract.gtk_h.g_file_input_stream_query_info_finish(HANDLE(), result.HANDLE(), GERROR);
        if (! java.util.Objects.equals(MemoryAddress.NULL, GERROR)) {
            throw new io.github.jwharm.javagi.interop.GErrorException(GERROR);
        }
        return new FileInfo(ProxyFactory.getProxy(RESULT, true));

...
public final class GstPlay {
    ...
    static {
        LibLoad.loadLibrary("gstplay-1.0");
        registerTypes();
    }
...
private static final MethodHandle gst_play_state_get_name = Interop.downcallHandle(
    ...
    private static void registerTypes() {
        if (Play.isAvailable()) Interop.register(Play.getType(), Play.fromAddress);
        if (PlayAudioInfo.isAvailable()) Interop.register(PlayAudioInfo.getType(), PlayAudioInfo.fromAddress);
        if (PlayMediaInfo.isAvailable()) Interop.register(PlayMediaInfo.getType(), PlayMediaInfo.fromAddress);
        if (PlaySignalAdapter.isAvailable()) Interop.register(PlaySignalAdapter.getType(), PlaySignalAdapter.fromAddress);
        if (PlayStreamInfo.isAvailable()) Interop.register(PlayStreamInfo.getType(), PlayStreamInfo.fromAddress);
        if (PlaySubtitleInfo.isAvailable()) Interop.register(PlaySubtitleInfo.getType(), PlaySubtitleInfo.fromAddress);
        if (PlayVideoInfo.isAvailable()) Interop.register(PlayVideoInfo.getType(), PlayVideoInfo.fromAddress);
        if (PlayVideoOverlayVideoRenderer.isAvailable()) Interop.register(PlayVideoOverlayVideoRenderer.getType(), PlayVideoOverlayVideoRenderer.fromAddress);
        if (PlayVideoRenderer.isAvailable()) Interop.register(PlayVideoRenderer.getType(), PlayVideoRenderer.fromAddress);
    }
}

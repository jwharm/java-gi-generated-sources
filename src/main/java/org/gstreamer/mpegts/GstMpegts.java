...
public final class GstMpegts {
    ...
    static {
        LibLoad.loadLibrary("gstmpegts-1.0");
        registerTypes();
    }
...
public static void dvbComponentDescriptorFree(org.gstreamer.mpegts.ComponentDescriptor source) {
    try {
        DowncallHandles.gst_mpegts_dvb_component_descriptor_free.invokeExact(source.handle());
    } catch (Throwable ERR) {
        throw new AssertionError("Unexpected exception occured: ", ERR);
    }
}
...
public static org.gstreamer.gst.Event eventNewMpegtsSection(org.gstreamer.mpegts.Section section) {
    MemoryAddress RESULT;
    try {
        RESULT = (MemoryAddress) DowncallHandles.gst_event_new_mpegts_section.invokeExact(section.handle());
    } catch (Throwable ERR) {
        throw new AssertionError("Unexpected exception occured: ", ERR);
    }
...
public static org.gstreamer.mpegts.Section eventParseMpegtsSection(org.gstreamer.gst.Event event) {
    MemoryAddress RESULT;
    try {
        RESULT = (MemoryAddress) DowncallHandles.gst_event_parse_mpegts_section.invokeExact(event.handle());
    } catch (Throwable ERR) {
        throw new AssertionError("Unexpected exception occured: ", ERR);
    }
    ...
    try (MemorySession SCOPE = MemorySession.openConfined()) {
        ...
        try {
            RESULT = (MemoryAddress) DowncallHandles.gst_mpegts_find_descriptor.invokeExact(
                    Interop.allocateNativeArray(descriptors, org.gstreamer.mpegts.Descriptor.getMemoryLayout(), false, SCOPE),
                    tag);
        } catch (Throwable ERR) {
            throw new AssertionError("Unexpected exception occured: ", ERR);
        }
        return org.gstreamer.mpegts.Descriptor.fromAddress.marshal(RESULT, null);
    }
}
...
public static org.gstreamer.mpegts.Section messageParseMpegtsSection(org.gstreamer.gst.Message message) {
    MemoryAddress RESULT;
    try {
        RESULT = (MemoryAddress) DowncallHandles.gst_message_parse_mpegts_section.invokeExact(message.handle());
    } catch (Throwable ERR) {
        throw new AssertionError("Unexpected exception occured: ", ERR);
    }
    var OBJECT = org.gstreamer.mpegts.Section.fromAddress.marshal(RESULT, null);
...
public static org.gstreamer.mpegts.SCTESIT scteCancelNew(int eventId) {
    MemoryAddress RESULT;
    try {
        RESULT = (MemoryAddress) DowncallHandles.gst_mpegts_scte_cancel_new.invokeExact(eventId);
    } catch (Throwable ERR) {
        throw new AssertionError("Unexpected exception occured: ", ERR);
    }
    var OBJECT = org.gstreamer.mpegts.SCTESIT.fromAddress.marshal(RESULT, null);
    OBJECT.takeOwnership();
    return OBJECT;
}

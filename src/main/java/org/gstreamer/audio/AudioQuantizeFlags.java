...
public class AudioQuantizeFlags extends io.github.jwharm.javagi.Bitfield {
     ...
     */
    public AudioQuantizeFlags or(AudioQuantizeFlags mask) {
        return new AudioQuantizeFlags(this.getValue() | mask.getValue());
    }
    ...
    public static AudioQuantizeFlags combined(AudioQuantizeFlags mask, AudioQuantizeFlags... masks) {
        int value = mask.getValue();        for (AudioQuantizeFlags arg : masks) {
            value |= arg.getValue();
        }

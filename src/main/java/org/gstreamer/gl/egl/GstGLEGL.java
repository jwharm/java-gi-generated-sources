...
public static java.lang.String eglGetErrorString(int err) {
    ...
    try {
        RESULT = (MemoryAddress) DowncallHandles.gst_egl_get_error_string.invokeExact(err);
    } catch (Throwable ERR) {
...
public static boolean isGlMemoryEgl(org.gstreamer.gst.Memory mem) {
    int RESULT;
    try {
        RESULT = (int) DowncallHandles.gst_is_gl_memory_egl.invokeExact(mem.handle());
    } catch (Throwable ERR) {
        throw new AssertionError("Unexpected exception occured: ", ERR);
    }
...
private static final MethodHandle gst_is_gl_memory_egl = Interop.downcallHandle(
    ...
    private static void registerTypes() {
        if (GLDisplayEGL.isAvailable()) Interop.register(GLDisplayEGL.getType(), GLDisplayEGL.fromAddress);
        if (GLDisplayEGLDevice.isAvailable()) Interop.register(GLDisplayEGLDevice.getType(), GLDisplayEGLDevice.fromAddress);
        if (GLMemoryEGLAllocator.isAvailable()) Interop.register(GLMemoryEGLAllocator.getType(), GLMemoryEGLAllocator.fromAddress);
    }
}

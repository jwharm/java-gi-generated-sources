...
public final class GstNet {
    ...
    static {
        LibLoad.loadLibrary("gstnet-1.0");
        registerTypes();
    }
...
public static org.gstreamer.net.NetAddressMeta bufferAddNetAddressMeta(org.gstreamer.gst.Buffer buffer, org.gtk.gio.SocketAddress addr) {
    ...
    return org.gstreamer.net.NetAddressMeta.fromAddress.marshal(RESULT, null);
}
...
public static org.gstreamer.net.NetControlMessageMeta bufferAddNetControlMessageMeta(org.gstreamer.gst.Buffer buffer, org.gtk.gio.SocketControlMessage message) {
    ...
    return org.gstreamer.net.NetControlMessageMeta.fromAddress.marshal(RESULT, null);
}
...
public static org.gstreamer.net.NetAddressMeta bufferGetNetAddressMeta(org.gstreamer.gst.Buffer buffer) {
    MemoryAddress RESULT;
    try {
        RESULT = (MemoryAddress) DowncallHandles.gst_buffer_get_net_address_meta.invokeExact(buffer.handle());
    } catch (Throwable ERR) {
        throw new AssertionError("Unexpected exception occured: ", ERR);
    }
    return org.gstreamer.net.NetAddressMeta.fromAddress.marshal(RESULT, null);
    ...
    try (MemorySession SCOPE = MemorySession.openConfined()) {
        ...
        try {
            RESULT = (int) DowncallHandles.gst_ptp_init.invokeExact(
                    clockId,
                    (Addressable) (interfaces == null ? MemoryAddress.NULL : Interop.allocateNativeArray(interfaces, false, SCOPE)));
        } catch (Throwable ERR) {
            throw new AssertionError("Unexpected exception occured: ", ERR);
        }
        return Marshal.integerToBoolean.marshal(RESULT, null).booleanValue();
...
public static void ptpStatisticsCallbackRemove(long id) {
    try {
        DowncallHandles.gst_ptp_statistics_callback_remove.invokeExact(id);
    } catch (Throwable ERR) {
        throw new AssertionError("Unexpected exception occured: ", ERR);
    }
}
...
private static final MethodHandle gst_ptp_statistics_callback_remove = Interop.downcallHandle(
    ...
    private static void registerTypes() {
        if (NetClientClock.isAvailable()) Interop.register(NetClientClock.getType(), NetClientClock.fromAddress);
        if (NetTimeProvider.isAvailable()) Interop.register(NetTimeProvider.getType(), NetTimeProvider.fromAddress);
        if (NtpClock.isAvailable()) Interop.register(NtpClock.getType(), NtpClock.fromAddress);
        if (PtpClock.isAvailable()) Interop.register(PtpClock.getType(), PtpClock.fromAddress);
    }
}

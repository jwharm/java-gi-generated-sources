...
public class DBusObjectSkeleton extends org.gtk.gobject.GObject implements org.gtk.gio.DBusObject {
     ...
     */
    protected DBusObjectSkeleton(Addressable address) {
        super(address);
    }
     ...
     */
    @ApiStatus.Internal
    public static final Marshal<Addressable, DBusObjectSkeleton> fromAddress = (input, scope) -> input.equals(MemoryAddress.NULL) ? null : new DBusObjectSkeleton(input);
    
    private static MemoryAddress constructNew(java.lang.String objectPath) {
        try (MemorySession SCOPE = MemorySession.openConfined()) {
            MemoryAddress RESULT;
            try {
                RESULT = (MemoryAddress) DowncallHandles.g_dbus_object_skeleton_new.invokeExact(Marshal.stringToAddress.marshal(objectPath, SCOPE));
            } catch (Throwable ERR) {
    ...
    /**
     * Functional interface declaration of the {@code AuthorizeMethod} callback.
     */
    @FunctionalInterface
    public interface AuthorizeMethod {
         ...
         */
        @ApiStatus.Internal MethodHandle HANDLE = Interop.getHandle(MethodHandles.lookup(), AuthorizeMethod.class, DESCRIPTOR);
    ...
    public Signal<DBusObjectSkeleton.AuthorizeMethod> onAuthorizeMethod(DBusObjectSkeleton.AuthorizeMethod handler) {
        ...
        try {
            var RESULT = (long) Interop.g_signal_connect_data.invokeExact(
                handle(), Interop.allocateNativeString("authorize-method", SCOPE), (Addressable) handler.toCallback(), (Addressable) MemoryAddress.NULL, (Addressable) MemoryAddress.NULL, 0);
    ...
    public static boolean isAvailable() {
        return DowncallHandles.g_dbus_object_skeleton_get_type != null;
    }
}

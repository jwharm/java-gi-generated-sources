...
public class Quaternion extends Struct {
    ...
    public static Quaternion allocate() {
        ...
        Quaternion newInstance = new Quaternion(segment.address());
     ...
     */
    protected Quaternion(Addressable address) {
        super(address);
    }
     ...
     */
    @ApiStatus.Internal
    public static final Marshal<Addressable, Quaternion> fromAddress = (input, scope) -> input.equals(MemoryAddress.NULL) ? null : new Quaternion(input);
    ...
    public static Quaternion alloc() {
        ...
        var OBJECT = org.gtk.graphene.Quaternion.fromAddress.marshal(RESULT, null);
    ...
    public org.gtk.graphene.Quaternion init(float x, float y, float z, float w) {
        ...
        return org.gtk.graphene.Quaternion.fromAddress.marshal(RESULT, null);
    }
    ...
    public void toAngleVec3(Out<Float> angle, org.gtk.graphene.Vec3 axis) {
        try (MemorySession SCOPE = MemorySession.openConfined()) {
            MemorySegment anglePOINTER = SCOPE.allocate(Interop.valueLayout.C_FLOAT);
            try {
    ...
    public void toAngles(Out<Float> degX, Out<Float> degY, Out<Float> degZ) {
        try (MemorySession SCOPE = MemorySession.openConfined()) {
            MemorySegment degXPOINTER = SCOPE.allocate(Interop.valueLayout.C_FLOAT);
            MemorySegment degYPOINTER = SCOPE.allocate(Interop.valueLayout.C_FLOAT);
            MemorySegment degZPOINTER = SCOPE.allocate(Interop.valueLayout.C_FLOAT);
            try {
    ...
    public void toRadians(Out<Float> radX, Out<Float> radY, Out<Float> radZ) {
        try (MemorySession SCOPE = MemorySession.openConfined()) {
            MemorySegment radXPOINTER = SCOPE.allocate(Interop.valueLayout.C_FLOAT);
            MemorySegment radYPOINTER = SCOPE.allocate(Interop.valueLayout.C_FLOAT);
            MemorySegment radZPOINTER = SCOPE.allocate(Interop.valueLayout.C_FLOAT);
            try {

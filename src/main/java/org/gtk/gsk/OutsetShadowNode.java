...
public class OutsetShadowNode extends org.gtk.gsk.RenderNode {
     ...
     * @return                    An instance of "OutsetShadowNode" that points to the memory address of the provided GObject.
    ...
    public static OutsetShadowNode castFrom(org.gtk.gobject.Object gobject) {
        if (org.gtk.gobject.GObject.typeCheckInstanceIsA(gobject.g_type_instance$get(), org.gtk.gobject.GObject.typeFromName("GskOutsetShadowNode"))) {
    ...
    private static Refcounted constructNew(@NotNull org.gtk.gsk.RoundedRect outline, @NotNull org.gtk.gdk.RGBA color, float dx, float dy, float spread, float blurRadius) {
        ...
        Refcounted RESULT;
        try {
            RESULT = Refcounted.get((MemoryAddress) DowncallHandles.gsk_outset_shadow_node_new.invokeExact(
                    outline.handle(),
                    color.handle(),
                    dx,
                    dy,

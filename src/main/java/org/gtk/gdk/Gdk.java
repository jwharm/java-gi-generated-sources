 ...
 * <p>
 * This can be used in {@link Drop#status} messages when any drop
...
/**
 * A mask covering all entries in {@code GdkModifierType}.
 */
 ...
 * <p>
 * It takes a render buffer ID ({@code source_type} == GL_RENDERBUFFER) or a texture
 * id ({@code source_type} == GL_TEXTURE) and draws it onto {@code cr} with an OVER operation,
 * respecting the current clip. The top left corner of the rectangle specified
 * by {@code x}, {@code y}, {@code width} and {@code height} will be drawn at the current (0,0) position of
 * the {@code cairo_t}.
 * <p>
 * This will work for <em>all</em> {@code cairo_t}, as long as {@code surface} is realized, but the
 * fallback implementation that reads back the pixels from the buffer may be
 * used in the general case. In the case of direct drawing to a surface with
 * no special effects applied to {@code cr} it will however use a more efficient
 * approach.
 * <p>
 ...
 * @param sourceType The type of the {@code source}
 * @param bufferScale The scale-factor that the {@code source} buffer is allocated for
 * @param x The source x position in {@code source} to start copying from in GL coordinates
 * @param y The source y position in {@code source} to start copying from in GL coordinates
 ...
 *   in GTK, use {@link GLTexture#GLTexture}; if you want to use that texture in
 *   Cairo, use {@link Texture#download} to download the data into a Cairo
 *   image surface.
 */
@Deprecated
...
public static org.cairographics.Region cairoRegionCreateFromSurface(org.cairographics.Surface surface) {
    ...
    var OBJECT = org.cairographics.Region.fromAddress.marshal(RESULT, null);
 ...
 * @param stream a {@code GInputStream} to read the serialized content from
 ...
 * @return {@code true} if the operation was successful. In this case,
 *   {@code value} is set. {@code false} if an error occurred. In this case,
 *   {@code error} is set
 ...
 * @param stream a {@code GOutputStream} to write the serialized content to
 * @param mimeType the mime type to serialize to
 ...
 * @param result the {@code GAsyncResult}
 * @return {@code true} if the operation was successful, {@code false} if an
 ...
 * @param event1 first {@code GdkEvent}
...
public static @Nullable java.lang.String internMimeType(java.lang.String string) {
    try (MemorySession SCOPE = MemorySession.openConfined()) {
        MemoryAddress RESULT;
        try {
            RESULT = (MemoryAddress) DowncallHandles.gdk_intern_mime_type.invokeExact(Marshal.stringToAddress.marshal(string, SCOPE));
        } catch (Throwable ERR) {
            throw new AssertionError("Unexpected exception occured: ", ERR);
        }
 ...
 * @param lower return location for lowercase version of {@code symbol}
...
public static void keyvalConvertCase(int symbol, Out<Integer> lower, Out<Integer> upper) {
    try (MemorySession SCOPE = MemorySession.openConfined()) {
        MemorySegment lowerPOINTER = SCOPE.allocate(Interop.valueLayout.C_INT);
        MemorySegment upperPOINTER = SCOPE.allocate(Interop.valueLayout.C_INT);
        try {
            ...
            throw new AssertionError("Unexpected exception occured: ", ERR);
                ...
                lower.set(lowerPOINTER.get(Interop.valueLayout.C_INT, 0));
                upper.set(upperPOINTER.get(Interop.valueLayout.C_INT, 0));
    }
}
...
public static int keyvalFromName(java.lang.String keyvalName) {
    try (MemorySession SCOPE = MemorySession.openConfined()) {
        int RESULT;
        try {
            RESULT = (int) DowncallHandles.gdk_keyval_from_name.invokeExact(Marshal.stringToAddress.marshal(keyvalName, SCOPE));
        } catch (Throwable ERR) {
            throw new AssertionError("Unexpected exception occured: ", ERR);
        }
        return RESULT;
 ...
 * @return {@code true} if {@code keyval} is in lower case, or if {@code keyval} is not
 ...
 * @param keyval a key value.
 * @return {@code true} if {@code keyval} is in upper case, or if {@code keyval} is not subject to
 *  case conversion.
 */
 ...
 * @param keyval a key value.
 * @return the lower case form of {@code keyval}, or {@code keyval} itself if it is already
 ...
 * @param keyval a key value.
 * @return the upper case form of {@code keyval}, or {@code keyval} itself if it is already
 ...
 * @param xOrigin X pixel where you intend to draw the layout with this clip
 * @param yOrigin Y pixel where you intend to draw the layout with this clip
 * @param indexRanges array of byte indexes into the layout, where even members of array are start indexes and odd elements are end indexes
 ...
 * @param xOrigin X pixel where you intend to draw the layout line with this clip
 * @param yOrigin baseline pixel where you intend to draw the layout line with this clip
 * @param indexRanges array of byte indexes into the layout, where even
    ...
    try (MemorySession SCOPE = MemorySession.openConfined()) {
        ...
        try {
            RESULT = (MemoryAddress) DowncallHandles.gdk_pango_layout_line_get_clip_region.invokeExact(
                    ...
                    xOrigin,
                    yOrigin,
                    Interop.allocateNativeArray(indexRanges, false, SCOPE),
                    nRanges);
        } catch (Throwable ERR) {
            throw new AssertionError("Unexpected exception occured: ", ERR);
        }
 ...
 * @param srcX Source X coordinate within {@code surface}
 * @param srcY Source Y coordinate within {@code surface}
...
public static @Nullable org.gtk.gdkpixbuf.Pixbuf pixbufGetFromSurface(org.cairographics.Surface surface, int srcX, int srcY, int width, int height) {
    ...
    var OBJECT = (org.gtk.gdkpixbuf.Pixbuf) Interop.register(RESULT, org.gtk.gdkpixbuf.Pixbuf.fromAddress).marshal(RESULT, null);
    OBJECT.takeOwnership();
    return OBJECT;
}
 ...
 * as {@link Display#open}, {@code gtk_init()}, or {@code gtk_init_check()}
...
public static void setAllowedBackends(java.lang.String backends) {
    try (MemorySession SCOPE = MemorySession.openConfined()) {
        try {
            DowncallHandles.gdk_set_allowed_backends.invokeExact(Marshal.stringToAddress.marshal(backends, SCOPE));
        } catch (Throwable ERR) {
            throw new AssertionError("Unexpected exception occured: ", ERR);
        }
    }
...
private static final MethodHandle gdk_cairo_draw_from_gl = Interop.downcallHandle(
        "gdk_cairo_draw_from_gl",
        FunctionDescriptor.ofVoid(Interop.valueLayout.ADDRESS, Interop.valueLayout.ADDRESS, Interop.valueLayout.C_INT, Interop.valueLayout.C_INT, Interop.valueLayout.C_INT, Interop.valueLayout.C_INT, Interop.valueLayout.C_INT, Interop.valueLayout.C_INT, Interop.valueLayout.C_INT),
        false
);

private static final MethodHandle gdk_cairo_rectangle = Interop.downcallHandle(
        "gdk_cairo_rectangle",
        FunctionDescriptor.ofVoid(Interop.valueLayout.ADDRESS, Interop.valueLayout.ADDRESS),
        false
);

private static final MethodHandle gdk_cairo_region = Interop.downcallHandle(
        "gdk_cairo_region",
        FunctionDescriptor.ofVoid(Interop.valueLayout.ADDRESS, Interop.valueLayout.ADDRESS),
        false
);

private static final MethodHandle gdk_cairo_region_create_from_surface = Interop.downcallHandle(
        "gdk_cairo_region_create_from_surface",
        FunctionDescriptor.of(Interop.valueLayout.ADDRESS, Interop.valueLayout.ADDRESS),
        false
);

private static final MethodHandle gdk_cairo_set_source_pixbuf = Interop.downcallHandle(
...
private static final MethodHandle gdk_cairo_set_source_rgba = Interop.downcallHandle(
        "gdk_cairo_set_source_rgba",
        FunctionDescriptor.ofVoid(Interop.valueLayout.ADDRESS, Interop.valueLayout.ADDRESS),
        false
);

private static final MethodHandle gdk_content_deserialize_async = Interop.downcallHandle(
...
private static final MethodHandle gdk_content_deserialize_finish = Interop.downcallHandle(
...
private static final MethodHandle gdk_content_register_deserializer = Interop.downcallHandle(
...
private static final MethodHandle gdk_content_register_serializer = Interop.downcallHandle(
        "gdk_content_register_serializer",
        FunctionDescriptor.ofVoid(Interop.valueLayout.C_LONG, Interop.valueLayout.ADDRESS, Interop.valueLayout.ADDRESS, Interop.valueLayout.ADDRESS, Interop.valueLayout.ADDRESS),
        false
);

private static final MethodHandle gdk_content_serialize_async = Interop.downcallHandle(
...
private static final MethodHandle gdk_content_serialize_finish = Interop.downcallHandle(
...
private static final MethodHandle gdk_intern_mime_type = Interop.downcallHandle(
        "gdk_intern_mime_type",
        FunctionDescriptor.of(Interop.valueLayout.ADDRESS, Interop.valueLayout.ADDRESS),
        false
);

private static final MethodHandle gdk_keyval_convert_case = Interop.downcallHandle(
        "gdk_keyval_convert_case",
        FunctionDescriptor.ofVoid(Interop.valueLayout.C_INT, Interop.valueLayout.ADDRESS, Interop.valueLayout.ADDRESS),
        false
);

private static final MethodHandle gdk_keyval_from_name = Interop.downcallHandle(
        "gdk_keyval_from_name",
        FunctionDescriptor.of(Interop.valueLayout.C_INT, Interop.valueLayout.ADDRESS),
        false
);

private static final MethodHandle gdk_keyval_is_lower = Interop.downcallHandle(
        "gdk_keyval_is_lower",
        FunctionDescriptor.of(Interop.valueLayout.C_INT, Interop.valueLayout.C_INT),
        false
);

private static final MethodHandle gdk_keyval_is_upper = Interop.downcallHandle(
        "gdk_keyval_is_upper",
        FunctionDescriptor.of(Interop.valueLayout.C_INT, Interop.valueLayout.C_INT),
        false
);

private static final MethodHandle gdk_keyval_name = Interop.downcallHandle(
        "gdk_keyval_name",
        FunctionDescriptor.of(Interop.valueLayout.ADDRESS, Interop.valueLayout.C_INT),
        false
);

private static final MethodHandle gdk_keyval_to_lower = Interop.downcallHandle(
        "gdk_keyval_to_lower",
        FunctionDescriptor.of(Interop.valueLayout.C_INT, Interop.valueLayout.C_INT),
        false
);

private static final MethodHandle gdk_keyval_to_unicode = Interop.downcallHandle(
        "gdk_keyval_to_unicode",
        FunctionDescriptor.of(Interop.valueLayout.C_INT, Interop.valueLayout.C_INT),
        false
);

private static final MethodHandle gdk_keyval_to_upper = Interop.downcallHandle(
        "gdk_keyval_to_upper",
        FunctionDescriptor.of(Interop.valueLayout.C_INT, Interop.valueLayout.C_INT),
        false
);

private static final MethodHandle gdk_paintable_new_empty = Interop.downcallHandle(
        "gdk_paintable_new_empty",
        FunctionDescriptor.of(Interop.valueLayout.ADDRESS, Interop.valueLayout.C_INT, Interop.valueLayout.C_INT),
        false
);

private static final MethodHandle gdk_pango_layout_get_clip_region = Interop.downcallHandle(
        "gdk_pango_layout_get_clip_region",
        FunctionDescriptor.of(Interop.valueLayout.ADDRESS, Interop.valueLayout.ADDRESS, Interop.valueLayout.C_INT, Interop.valueLayout.C_INT, Interop.valueLayout.ADDRESS, Interop.valueLayout.C_INT),
        false
);

private static final MethodHandle gdk_pango_layout_line_get_clip_region = Interop.downcallHandle(
        "gdk_pango_layout_line_get_clip_region",
        FunctionDescriptor.of(Interop.valueLayout.ADDRESS, Interop.valueLayout.ADDRESS, Interop.valueLayout.C_INT, Interop.valueLayout.C_INT, Interop.valueLayout.ADDRESS, Interop.valueLayout.C_INT),
        false
);

private static final MethodHandle gdk_pixbuf_get_from_surface = Interop.downcallHandle(
...
private static final MethodHandle gdk_pixbuf_get_from_texture = Interop.downcallHandle(
        "gdk_pixbuf_get_from_texture",
        FunctionDescriptor.of(Interop.valueLayout.ADDRESS, Interop.valueLayout.ADDRESS),
        false
);

private static final MethodHandle gdk_set_allowed_backends = Interop.downcallHandle(
...
private static final MethodHandle gdk_toplevel_size_get_type = Interop.downcallHandle(
        "gdk_toplevel_size_get_type",
        FunctionDescriptor.of(Interop.valueLayout.C_LONG),
        false
);

private static final MethodHandle gdk_unicode_to_keyval = Interop.downcallHandle(
...
private static final MethodHandle gdk_vulkan_error_quark = Interop.downcallHandle(
    ...
    private static void registerTypes() {
        if (AppLaunchContext.isAvailable()) Interop.register(AppLaunchContext.getType(), AppLaunchContext.fromAddress);
        if (ButtonEvent.isAvailable()) Interop.register(ButtonEvent.getType(), ButtonEvent.fromAddress);
        if (CairoContext.isAvailable()) Interop.register(CairoContext.getType(), CairoContext.fromAddress);
        if (Clipboard.isAvailable()) Interop.register(Clipboard.getType(), Clipboard.fromAddress);
        if (ContentDeserializer.isAvailable()) Interop.register(ContentDeserializer.getType(), ContentDeserializer.fromAddress);
        if (ContentProvider.isAvailable()) Interop.register(ContentProvider.getType(), ContentProvider.fromAddress);
        if (ContentSerializer.isAvailable()) Interop.register(ContentSerializer.getType(), ContentSerializer.fromAddress);
        if (CrossingEvent.isAvailable()) Interop.register(CrossingEvent.getType(), CrossingEvent.fromAddress);
        if (Cursor.isAvailable()) Interop.register(Cursor.getType(), Cursor.fromAddress);
        if (DNDEvent.isAvailable()) Interop.register(DNDEvent.getType(), DNDEvent.fromAddress);
        if (DeleteEvent.isAvailable()) Interop.register(DeleteEvent.getType(), DeleteEvent.fromAddress);
        if (Device.isAvailable()) Interop.register(Device.getType(), Device.fromAddress);
        if (DeviceTool.isAvailable()) Interop.register(DeviceTool.getType(), DeviceTool.fromAddress);
        if (Display.isAvailable()) Interop.register(Display.getType(), Display.fromAddress);
        if (DisplayManager.isAvailable()) Interop.register(DisplayManager.getType(), DisplayManager.fromAddress);
        if (Drag.isAvailable()) Interop.register(Drag.getType(), Drag.fromAddress);
        if (DrawContext.isAvailable()) Interop.register(DrawContext.getType(), DrawContext.fromAddress);
        if (Drop.isAvailable()) Interop.register(Drop.getType(), Drop.fromAddress);
        if (Event.isAvailable()) Interop.register(Event.getType(), Event.fromAddress);
        if (FocusEvent.isAvailable()) Interop.register(FocusEvent.getType(), FocusEvent.fromAddress);
        if (FrameClock.isAvailable()) Interop.register(FrameClock.getType(), FrameClock.fromAddress);
        if (GLContext.isAvailable()) Interop.register(GLContext.getType(), GLContext.fromAddress);
        if (GLTexture.isAvailable()) Interop.register(GLTexture.getType(), GLTexture.fromAddress);
        if (GrabBrokenEvent.isAvailable()) Interop.register(GrabBrokenEvent.getType(), GrabBrokenEvent.fromAddress);
        if (KeyEvent.isAvailable()) Interop.register(KeyEvent.getType(), KeyEvent.fromAddress);
        if (MemoryTexture.isAvailable()) Interop.register(MemoryTexture.getType(), MemoryTexture.fromAddress);
        if (Monitor.isAvailable()) Interop.register(Monitor.getType(), Monitor.fromAddress);
        if (MotionEvent.isAvailable()) Interop.register(MotionEvent.getType(), MotionEvent.fromAddress);
        if (PadEvent.isAvailable()) Interop.register(PadEvent.getType(), PadEvent.fromAddress);
        if (ProximityEvent.isAvailable()) Interop.register(ProximityEvent.getType(), ProximityEvent.fromAddress);
        if (ScrollEvent.isAvailable()) Interop.register(ScrollEvent.getType(), ScrollEvent.fromAddress);
        if (Seat.isAvailable()) Interop.register(Seat.getType(), Seat.fromAddress);
        if (Snapshot.isAvailable()) Interop.register(Snapshot.getType(), Snapshot.fromAddress);
        if (Surface.isAvailable()) Interop.register(Surface.getType(), Surface.fromAddress);
        if (Texture.isAvailable()) Interop.register(Texture.getType(), Texture.fromAddress);
        if (TouchEvent.isAvailable()) Interop.register(TouchEvent.getType(), TouchEvent.fromAddress);
        if (TouchpadEvent.isAvailable()) Interop.register(TouchpadEvent.getType(), TouchpadEvent.fromAddress);
        if (VulkanContext.isAvailable()) Interop.register(VulkanContext.getType(), VulkanContext.fromAddress);
        if (DevicePad.isAvailable()) Interop.register(DevicePad.getType(), DevicePad.fromAddress);
        if (DragSurface.isAvailable()) Interop.register(DragSurface.getType(), DragSurface.fromAddress);
        if (Paintable.isAvailable()) Interop.register(Paintable.getType(), Paintable.fromAddress);
        if (Popup.isAvailable()) Interop.register(Popup.getType(), Popup.fromAddress);
        if (Toplevel.isAvailable()) Interop.register(Toplevel.getType(), Toplevel.fromAddress);
    }
}

...
public class ContentProvider extends org.gtk.gobject.GObject {
     ...
     */
    protected ContentProvider(Addressable address) {
        super(address);
    }
     ...
     */
    @ApiStatus.Internal
    public static final Marshal<Addressable, ContentProvider> fromAddress = (input, scope) -> input.equals(MemoryAddress.NULL) ? null : new ContentProvider(input);
    ...
    private static MemoryAddress constructNewForValue(org.gtk.gobject.Value value) {
        MemoryAddress RESULT;
        try {
            RESULT = (MemoryAddress) DowncallHandles.gdk_content_provider_new_for_value.invokeExact(value.handle());
        } catch (Throwable ERR) {
            throw new AssertionError("Unexpected exception occured: ", ERR);
        }
        return RESULT;
        ...
        try (MemorySession SCOPE = MemorySession.openConfined()) {
            ...
            try {
                RESULT = (MemoryAddress) DowncallHandles.gdk_content_provider_new_union.invokeExact(
                        (Addressable) (providers == null ? MemoryAddress.NULL : Interop.allocateNativeArray(providers, false, SCOPE)),
                        nProviders);
            } catch (Throwable ERR) {
                throw new AssertionError("Unexpected exception occured: ", ERR);
            }
    ...
    /**
     * Functional interface declaration of the {@code ContentChanged} callback.
     */
    @FunctionalInterface
    public interface ContentChanged {
         ...
         */
        @ApiStatus.Internal MethodHandle HANDLE = Interop.getHandle(MethodHandles.lookup(), ContentChanged.class, DESCRIPTOR);
    ...
    public Signal<ContentProvider.ContentChanged> onContentChanged(ContentProvider.ContentChanged handler) {
        ...
        try {
            var RESULT = (long) Interop.g_signal_connect_data.invokeExact(
                handle(), Interop.allocateNativeString("content-changed", SCOPE), (Addressable) handler.toCallback(), (Addressable) MemoryAddress.NULL, (Addressable) MemoryAddress.NULL, 0);
    ...
    public static boolean isAvailable() {
        return DowncallHandles.gdk_content_provider_get_type != null;
    }
}

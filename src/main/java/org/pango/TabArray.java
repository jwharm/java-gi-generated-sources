...
public class TabArray extends Struct {
    ...
    public static TabArray allocate() {
        ...
        TabArray newInstance = new TabArray(segment.address());
     ...
     */
    protected TabArray(Addressable address) {
        super(address);
    }
     ...
     */
    @ApiStatus.Internal
    public static final Marshal<Addressable, TabArray> fromAddress = (input, scope) -> input.equals(MemoryAddress.NULL) ? null : new TabArray(input);
    ...
    public void getTab(int tabIndex, @Nullable Out<org.pango.TabAlign> alignment, Out<Integer> location) {
        try (MemorySession SCOPE = MemorySession.openConfined()) {
            MemorySegment alignmentPOINTER = SCOPE.allocate(Interop.valueLayout.C_INT);
            MemorySegment locationPOINTER = SCOPE.allocate(Interop.valueLayout.C_INT);
            try {
    ...
    public void getTabs(@Nullable Out<org.pango.TabAlign> alignments, int[] locations) {
        try (MemorySession SCOPE = MemorySession.openConfined()) {
            MemorySegment alignmentsPOINTER = SCOPE.allocate(Interop.valueLayout.C_INT);
            try {
                DowncallHandles.pango_tab_array_get_tabs.invokeExact(
                        ...
                        (Addressable) (locations == null ? MemoryAddress.NULL : Interop.allocateNativeArray(locations, false, SCOPE)));
            } catch (Throwable ERR) {
                throw new AssertionError("Unexpected exception occured: ", ERR);
            }

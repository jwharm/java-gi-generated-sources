...
public interface GtkTextCharPredicate {
    ...
    static MemorySegment allocate(GtkTextCharPredicate fi, MemorySession session) {
        return RuntimeHelper.upcallStub(GtkTextCharPredicate.class, fi, constants$2431.GtkTextCharPredicate$FUNC, session);
    }
    static GtkTextCharPredicate ofAddress(MemoryAddress addr, MemorySession session) {
        ...
        return (int _ch, java.lang.foreign.MemoryAddress _user_data) -> {
            try {
                return (int)constants$2431.GtkTextCharPredicate$MH.invokeExact((Addressable)symbol, _ch, (java.lang.foreign.Addressable)_user_data);
            } catch (Throwable ex$) {
